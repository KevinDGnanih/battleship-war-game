...
class Game:
    ...
    def __init__(self, num_ships, size):
        self.num_ships = num_ships
        self.size = size
    ...
    def game_over(self):
        """
        Checks if either player has sunk the other player's ships
        """
        if self.scores["player"] >= self.num_ships or self.scores["ai"] >= self.num_ships:
            return True
    ...
    #       Player guesses
            ...
            while not player_board.valid_guess(row, column):
                row, column = player_board.ask_guess()
    ...
    #       Scores section
            print("+", "-" * 35, "+")
            print(f"After this round, the scroes are:\n {player_name}: {self.scores['player']}. AI: {self.scores['ai']}")
            print("+", "-" * 35, "+")
